...
- A server.R file containing a call to shinyServer()    (Correct)

- A shiny.R file containing calls to shinyServer() and shinyUI()

- A ui.R file containing a call to shinyUI()     (Correct)
...
- Missing a comma in the sidebar panel   (Correct)
...
Explanation -> All sidebar entries must be separed with comma.
...
- The server.R output name isn't the same as the plotOutput command used in ui.R.  (Correct)
...
Explanation -> ui.R plot name is newHist and server.R plot name is myHist
...
- Shiny Gadgets are designed to have small user interfaces that fit on one page.    (Correct)

- Shiny Gadgets are designed to be used by R users in the middle of a data analysis.    (Correct)
...
- No arguments are defined for pickXY()              (Correct)

- The input data is defined in such a way that it is not compatible with pickXY()


Explanation -> In function definition arguments place is empty.